...
export default class Addresses {
  ...
  async newAddress(identity: Types.Classes.CUser, input: any) {
    try {
      ...
      if (!payload.validate() || this.validateAddress(payload)) {
        const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_NEW_ADDRESS_INVALID_ADDRESS);
      ...
      const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_NEW_ADDRESS_EXCEPTION, exception);
      return error.logAndReturn(this.logger);
    }
  }
  ...
  async getAddressModels(identity: Types.Classes.CUser) {
    ...
    if (!contractModel) {
      const error = new Utils.iKomidaError(Utils.iKomidaError.IKOMIDA_USERS_SERVICE_ADDRESS_GET_ADDRESS_INVALID_CONTRACT);
      return error.logAndReturn(this.logger);
    }
